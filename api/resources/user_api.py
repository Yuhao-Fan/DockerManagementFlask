...
class UserApi(Resource):
    ...
    def patch(self):
        ...
        ret = u_dao.updateOneResourceById(user)
        if ret is None:
            return {"rep_code": HTTPStatus.CONFLICT, "msg": "Delete unsuccessfully, there would be no user with [{}] on file".format(user["id"])}, int(HTTPStatus.CONFLICT)
        return {"rep_code": HTTPStatus.OK, "msg": "The user[{}] is created successfully".format(user)}, int(HTTPStatus.OK)